...
{
    /* DMA配置 */
    dma_basic_config(dma_streamx, ch, par, mar, 0); /* 外设地址/存储器地址/存储器增量模式 */
 ...
 * @retval      无
 */
void dma_basic_config(DMA_Stream_TypeDef *dma_streamx, uint8_t ch, uint32_t par, uint32_t m0ar, uint32_t m1ar)
{
    /* DMA时钟使能 */
    ...
    /* DMA配置 */
    dma_streamx->CR &= ~(0x3 << 25);    /* 清除通道 */
    dma_streamx->CR |= ch << 25;        /* 通道设置 */
    
    dma_streamx->PAR = par;             /* 外设地址 */
    dma_streamx->M0AR = m0ar;           /* 存储器地址 */
    dma_streamx->M1AR = m1ar;           /* 存储器地址 */
